...
/* ===== Accept Page (single, clean implementation) ===== */
...
<style>
  ...
  .card{background:#1b1b1b;padding:40px 30px;border-radius:12px;max-width:520px;width:90%;text-align:center;
...
<head>
  ...
  <style>
    body { background:#0f0f0f; color:#fff; font-family:system-ui,-apple-system,Segoe UI,Roboto,sans-serif;
           display:flex;align-items:center;justify-content:center; height:100vh; margin:0; }
    .container { width:90%; max-width:440px; text-align:center; background:#1b1b1b; padding:40px 30px;
                 border-radius:12px; box-shadow:0 0 30px rgba(0,0,0,0.4); }
    h2 { font-size:2rem; margin-bottom:.75rem; }
    p  { font-size:1.05rem; color:#bbb; margin-bottom:1.5rem; }
    input[type="password"] { width:100%; padding:14px; border:none; border-radius:8px; background:#2b2b2b;
                             color:#fff; font-size:1.05rem; margin-bottom:1rem; box-sizing:border-box; }
    input[type="password"]:focus { outline:2px solid #ff5c33; }
    button { width:100%; padding:14px; border:none; border-radius:8px; background:#ff5c33; color:#fff;
             font-size:1.05rem; font-weight:700; cursor:pointer; }
    button:hover { background:#ff704d; }
    #msg { margin-top:.9rem; font-size:.95rem; color:#9aa0a6; min-height:1.2em; }
  </style>
</head>
<body>
  <div class="container">
    ...
    document.getElementById('f').addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (!j || !j.ok) {
          ...
          msg.textContent =
            code === 'invite_already_accepted' ? 'This invitation was already accepted. Please download the app and log in.' :
            code === 'invalid_token'           ? 'Invalid or expired link.' :
            code === 'invite_cancelled'        ? 'This invitation was cancelled.' :
            'Error saving password.';
          return;
        }
        msg.textContent = 'Password saved. Redirecting...';
        setTimeout(() => { window.location.href = ${JSON.stringify(DOWNLOAD_BASE_URL)}; }, 900);
      } catch {
...
/* ===== Accept API (JSON only; with proper already-accepted handling) ===== */
app.post('/api/invites/accept', async (req, res) => {
  try {
    ...
    await pool.query(
      `update invites set status='accepted', accepted_at=now() where token=$1`,
      [token]
    );
